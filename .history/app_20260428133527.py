...
with st.sidebar:
    ...
    api_key = st.text_input("🔑 OpenAI API Key", type="password", placeholder="sk-...", help="Get from https://platform.openai.com/api-keys")
    ...
    st.markdown('<div class="custom-divider"></div>', unsafe_allow_html=True)
    st.markdown('<div style="font-size:0.7rem;color:#8899aa;text-align:center;">Built with OpenAI GPT · FAISS · RAG · SQLite</div>', unsafe_allow_html=True)
...
if not api_key:
    st.markdown('<div class="warn-banner">✅ PDF uploaded! Now enter your OpenAI API key in the sidebar to process it.</div>', unsafe_allow_html=True)
    st.stop()
...
if "faiss_index" not in st.session_state or st.session_state.get("current_file") != filename:
    with st.spinner("🔍 Building semantic index..."):
        index, stored_chunks = build_faiss_index(client, chunks)
